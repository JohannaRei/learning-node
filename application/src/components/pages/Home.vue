<template>
  <main class="l-home-page">
    ...
    <div class="l-home">
      <h4 class="white--text text-xs-center my-0">Focus Budget Manager</h4>
    ...
    <v-fab-transition>
      <v-speed-dial v-model="fab" bottom right fixed direction="top" transition="scale-transition">
        <v-btn slot="activator" color="red lighten-1" dark fab v-model="fab">
          <v-icon>add</v-icon>
          <v-icon>close</v-icon>
        </v-btn>
  ...
  watch: {
    search: function() {
      if (this.search !== null || this.search !== "") {
        ...
        const results = this.budgets.filter(budget =>
          budget.client.match(regex)
        );
